    ...
    try {
      if (selectedOption === 'appointment') {
        ...
        if (result.success) {
          ...
          // Salvedad: Redirigir a otras gestiones
          if (result.redirect === 'other_services') {
            setPatientData({ documento: documentNumber });
            setCurrentScreen('other-services');
            
            let message = '';
            if (result.error === 'no_patient_record') {
              message = 'Paciente no encontrado en el sistema. Puede dirigirse a una secretaría.';
            } else if (result.error === 'no_appointment') {
              message = result.message || 'Usted no cuenta con turno programado';
            } else {
              message = 'Puede dirigirse a una secretaría para asistencia.';
            }
            ...
            toast({
              ...
              description: "No se encontró un turno asignado para el documento ingresado.",
              variant: "destructive",
            });
          }
        }
